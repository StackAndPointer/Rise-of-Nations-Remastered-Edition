...
/**
 * @brief 将 BitMask<806> 等伪模板结构体重命名为 BitMask_806 等普通结构体
 *
 * 并用 typedef 兼容原有用法
 */
...
/**
 * @brief 兼容原 BitMask<256> 用法
 */
#define BitMask BitMask_

/**
 * @brief 例如：BitMask<256> 替换为 BitMask_256
 *
 * 结构体成员声明处：
...
/**
 * @brief 例如：BitMask<806> 替换为 BitMask_806
 *
 * 结构体成员声明处：
 * BitMask_806 tech;
...
/**
 * @brief 站点数组基类，继承自ArrayBaseMaster，专门用于存储Site对象
 */
...
/**
 * @brief 站点简单复制数组，继承自ArrayBaseSite，可能用于浅拷贝
 */
struct  ArrayBaseSimpleCopySite : ArrayBaseSite
{
};
/**
 * @brief 无额外成员，继承基类的所有功能
 */

/**
 * @brief 站点数组，继承自ArrayBaseSimpleCopySite，添加当前索引
 */
...
/**
 * @brief 站点集合，继承自ArraySite，可能表示一组站点
 */
struct  Sites : ArraySite
{
};
/**
 * @brief 无额外成员，直接使用ArraySite的功能
 */



/**
 * @brief 整型数组基类，继承自ArrayBaseMaster，专门用于存储整数
 */
...
/**
 * @brief 整型简单复制数组，继承自ArrayBaseInt
 */
...
/**
 * @brief 简单整型数组，继承自ArrayBaseSimpleCopyInt，添加当前索引
 */
...
/**
 * @brief 字符串数据联合体，可以指向动态字符串或常量字符串
 *
...
/**
 * @brief 重命名说明：原字段名___u0是编译器生成的随机名
 *
 * 重命名为content，明确表示这是字符串内容的核心数据
 */

/**
 * @brief 领导者数据加密结构，可能用于存储加密的玩家数据
 */
...
/**
 * @brief 类型索引枚举，定义游戏中所有资源、单位、建筑等的类型标识
 */
 ...
 */
	...
	int handicap; /**< 游戏里的让分技巧，AI资源加成也通过此实现，有获取技巧的函数，是AI就+xx所以查看AI这项数值还是0 */
...
/**
 * @brief 领导者输出类 - 继承自LeaderData，8字节对齐
 */
struct  __declspec(align(8)) LeaderOut : LeaderData
{
};
/**
 * @brief 继承LeaderData的所有成员
 *
 * 8字节对齐确保在内存中的高效访问
 * 可能用于存储领导者的输出数据或状态信息
 */
...
/**
 * @brief 领导者类 - 多重继承自LeaderOut和GameAccess，4字节对齐
 */
...
{
	int unk; /**< 虚函数表指针 ILeaderboardsListener_vtbl* __vftable  */
};

/**
 * @brief 游戏设置结构体 - 存储多人游戏的各种设置选项
 *
...
/**
 * @brief 游戏设置联合体 - 允许以结构体或字节数组方式访问设置数据
 *
...
/**
 * @brief 重命名说明：原字段名__s0是编译器生成的随机名
 *
...
{
	int unk; /**< 虚函数表指针 NetPlayer_vtbl* __vftable */
};

/**
 * @brief 玩家数据结构体 - 存储单个玩家的所有游戏数据
 */
...
/**
 * @brief 游戏MOD结构体 - 游戏中的MOD管理
 */
struct  GameMod : GameAccess
{
	int unk; /**< 虚函数表指针 GameMod_vtbl* __vftable */
...
/**
 * @brief 重命名说明：原字段名___u7是编译器生成的随机名
 *
...
/**
 * @brief 游戏主结构体 - 继承自排行榜监听器和游戏访问接口，管理整个游戏状态
 */
...
/**
 * @brief 领导者选项输出结构 - 继承自LeaderOptionData，用于数据输出
 */
...
/**
 * @brief 领导者选项命令 - 特殊的命令结构，包含领导者选项数据
 */
...
{
	int unk; /**< 虚函数表指针 ArrayBase<Army*>_vtbl* __vftable */
	Army** list; /**< 军队指针数组 */
...
/**
 * @brief 支持简单复制操作的军队指针数组
 */

/**
 * @brief 军队指针数组 - 添加当前索引支持
 */
...
struct  Log
{
	int unk; /**< 虚函数表指针 Log_vtbl* __vftable */
	int flags; /**< 日志标志位 */
...
/**
 * @brief 数据遍历器 - 用于遍历游戏数据结构
 */
struct  DataWalk
{
	int unk; /**< 虚函数表指针 DataWalk_vtbl* __vftable  */
	int input; /**< 输入数据 */
	int checksum; /**< 校验和 */
	int flags; /**< 遍历标志位 */
};
...
/**
 * @brief 类型数据基类 - 存储游戏对象类型的通用属性
 */
struct  TypeData : GameAccessConst
{
	int unk; /**< 虚函数表指针 TypeData_vtbl* __vftable  */
	TypeIndex type; /**< 类型索引 */
...
struct Tribe : GameAccessConst
{
	...
	TribeIndex tribe; /**< 部落ID（对应TribeIndex枚举值） */
...
/**
 * @brief 缓冲区类（继承自ImageIO和BufferBase）
 */
...
/**
 * @brief 纹理类（继承自ImageIO和TextureBase）
 */
...
/**
 * @brief 重力窗口（继承自IFaceWindow，具有动画效果）
 */
...
/**
 * @brief 精灵类（继承自ImageIO和SpriteBase）
 */
...
/**
 * @brief 弹出窗口（继承自BasePop和GameAccessConst）
 */
...
/**
 * @brief 帮助框（继承自GraphicWin和MiscAccess）
 */
...
/**
 * @brief 设置窗口子窗口（继承自GravWindow和GameAccess）
 */
...
/**
 * @brief 自定义设置窗口（继承自SetupWinChild）
 */
...
/**
 * @brief 设置窗口（继承自HelpBoxWin、ILeaderboardsForELOListener和GameAccess）
 */
...
{
	int unk; /**< ArrayBase<RandomLogEntry>_vtbl* __vftable ; */
...
struct  XMLNode
{
	int unk; /**< XMLNode_vtbl* __vftable ; // 虚函数表指针 */
...
{
	int unk; /**< ScriptFuncSet_vtbl* __vftable ; */
	String name;
	int begin_func;
	int end_func;
	int index;
...
struct  __declspec(align(4)) ScriptParam
{
	String name;
...
struct  __declspec(align(4)) SymType
{
	int unk;//SymType_vtbl* __vftable ;
	SymType* of_type;
	int type;
	int index;
	String token;
	unsigned __int8 dont_delete;
};

struct  CallObj
{
	int unk;//CallObj_vtbl* __vftable ;
};

struct  __declspec(align(4)) ArrayBaseScriptParam : ArrayBaseMaster
{
	int unk;//ArrayBase<ScriptParam>_vtbl* __vftable;
	ScriptParam* list;
	unsigned __int8 flags;
};

const struct  ObjectArrayScriptParam : ArrayBaseScriptParam
{
};
...
struct  ScriptFunc : SymType, CallObj
{
	ObjectArrayScriptParam params;
	int return_type;
	SymType* sym_return_type;
	int address;
	ScriptFunc* next_overload;
	unsigned __int8 flags;
